...
@section('content')
    <div class="box box-primary">
        <div class="box-header">
            <h3 class="box-title">
                <a href="{{route('parlamentares.create')}}" class="btn btn-primary btn-lg"><i class="fa fa-plus"></i> NOVA PARLAMENTAR</a>
            </h3>
            <div class="box-tools">
                <form role="form" method="get" action="{{url('painel/parlamentares/pesquisar')}}" enctype="multipart/form-data">
                    {{--{{ csrf_field() }}--}}
        ...
        <div class="box-body table-responsive no-padding">
            ...
            <table class="table table-hover">
                ...
                @forelse($datas as $data)
                    <tr>
                        <td>{{$data->nome_parlamentar}}</td>
                        <td>{{$data->user->name. " ".$data->user->last_name}}</td>
                        <td>{{$data->sigla_partido." (". $data->nome_partido.")"}}</td>
                        <td>{{$data->status == 'A'? 'ATIVO':'INATIVO'}} </td>
                        <td>
                            <a href='{{route('parlamentares.edit', $data->id)}}' class="btn btn-success btn-xs"><i class="fa fa-edit"></i></a>
                            <a href="{{route('parlamentares.show', $data->id)}}" class="btn btn-info btn-xs"><i class="fa fa-eye"></i></a>
                        </td>
                    </tr>
                    @empty
                    <div class="box-body">
